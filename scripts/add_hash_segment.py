...
def AddHashSegmentToImage(file,ImageType,ScriptPath):
	...
	process = subprocess.Popen("python %s/pil-splitter.py %s ./Temp/split" % (ScriptPath,file), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	output, error = process.communicate()
	if error != "":
		print "Error: " + error
		sys.exit(1)
