...
void sierra_smem_reset_type_set(unsigned int reset_type)
{
  ...
  if (virtual_addr)
  {
    ...
    b2amsgp->in.brstsetflg  = BS_BCMSG_RTYPE_IS_SET;
    b2amsgp->crc32 = crc32(~0, (void *)b2amsgp, BC_MSG_CRC_SZ);
  }
  ...
  uint32 bytesleft)
{
  ...
  do                            /* break out of this loop in case of failure */
  {
    ...
    else if (imagetype == CWE_IMAGE_TYPE_BOOT)
    {
      ...
      {
        /* 1.1 then copy whole BOOT image to  SCRATCH_REGION_256 */
        /* In this case FULL BOOT image has already been stored in SCRATCH_REGION_256 */
...
/************
 *
 * Name:     sierra_smem_reset_type_flag_get
 *
 * Purpose:  get reset type flag from SMEM
 *
 * Parms:    none
 *
 ...
 *
 ************/
unsigned int sierra_smem_reset_type_flag_get(void)
{
  struct bc_smem_message_s *b2amsgp;
  unsigned char *virtual_addr;
...
unsigned int sierra_smem_err_fatal_count_get(void)
{
  ...
  dprintf(INFO, "sierra_smem_err_fatal_count_get: err_fatal_count=%d\n", err_fatal_count);
...
_local void bl_latest_err_count_get(uint32 *err_count)
{
  struct bc_smem_message_s *b2amsgp;
  unsigned int i;
  unsigned char *virtual_addr;
...
#ifdef SIERRA_IM_SM_TEST
      dprintf(INFO, "bl_latest_err_count_get: DDR err_count=%u IM err_count=%u\n", b2amsgp->out.recover_cnt, *err_count);
#endif
    }
  }
...
/************
 *
 * Name:     bl_im_smem_base_addr_get
 *
 * Purpose:  get IM SMEM base address
 *
 * Parms:    none
 *
 * Return:   IM SMEM base address
 *
 * Abort:    none
 *
 * Notes:    none
 *
 ************/
_local struct im_swap_data_s *bl_im_smem_base_addr_get(void)
{
  return (struct im_swap_data_s *)IM_SMEM_BASE;
}

/************
 *
 * Name:     bl_save_ddr_sm_to_im_sm
 *
 * Purpose:  Save the DDR SMEM data to IM SMEM
 *
 * Parms:    None
 *
 * Return:   None
  ...
  void)
{
  ...
  bl_latest_err_count_get(&(smem_p->recover_cnt));

  /*5. Update CRC */
  smem_p->crc32 = crcrc32((void *)smem_p, len, CRSTART_CRC32);
